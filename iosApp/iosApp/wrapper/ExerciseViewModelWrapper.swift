...
class ExerciseViewModelWrapper: ObservableObject {
    ...
    func fetchRecommendation() = viewModel.fetchRecommendation()
    func toggleTimer() = viewModel.toggleTimer()
    func setSelectedExercise(_ name: String) = viewModel.setSelectedExercise(name: name)
    func sendFeedback(successful: Bool) = viewModel.sendFeedback(successful: successful)
    ...
    func saveAll() = viewModel.saveAll()

    func totalWeight() -> Int32 { Int32(viewModel.totalWeight()) }
    func totalReps() -> Int32 { Int32(viewModel.totalReps()) }
    func totalSets() -> Int32 { Int32(viewModel.totalSets()) }
    func averageDuration() -> Int32 { Int32(viewModel.averageDuration()) }
}
